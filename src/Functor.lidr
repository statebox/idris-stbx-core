...
All the preliminary work is done, let's now start with the real stuff. As we said, a functor between categories $\mathcal{C}$ and $\mathcal{D}$, formally denoted with $F:\mathcal{C} \to \mathcal{D}$, is a mapping. We begin by noticing how a category consists of objects and morphisms, so our functor will have to act both on objects *and* morphisms.
