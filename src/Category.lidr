...
In practice, we need to describe, for any pair of objects $a$ and $b$, the collection of *arrows* (also called *morphisms*) going from $a$ to $b$. An arrow from $a$ to $b$ is sometimes mathematically denoted as $f:a \to b$ or more compactly as $a \xrightarrow{f} b$.
Moreover, if we denote a category as $\mathcal{C}$, a convenient notation to denote *all* the arrows from object $a$ to object $b$ is $\mathcal{C}(a,b)$.
